...
where Pin: ADCPin,
	...
	pub fn read(&mut self) -> Result<i16, EspError> {
		...
		let scaled =
			if mv < low_min { i16::MIN }
			else if mv < low_max { ((low_max - mv) as i32 * i16::MIN as i32 / (low_max - low_min) as i32) as i16 }
			else if mv < high_min { 0 }
			else if mv < high_max { ((mv - high_min) as i32 * i16::MAX as i32 / (high_max - high_min) as i32) as i16 }
			else { i16::MAX };
		Ok(scaled)
	}
}
