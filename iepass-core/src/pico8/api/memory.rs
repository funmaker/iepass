...
pub fn poke<A: Allocator + Clone + 'static>(env: EnvHandle<A>, (addr, bytes): (i16, Variadic<alloc::vec::Vec<u8>>)) -> Result<(), RuntimeError> {
...
pub fn peek<A: Allocator + Clone + 'static>(env: EnvHandle<A>, (addr, n): (i16, Option<i16>)) -> Result<Variadic<alloc::vec::Vec<u8>>, RuntimeError> {
	let env = env.borrow();
	let addr = addr.cast_unsigned() as usize;
