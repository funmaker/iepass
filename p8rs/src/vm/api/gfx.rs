...
use crate::vm::memory::machine_state::{MachineState, Palette};
...
pub fn set_cursor_color(draw_state: &mut MachineState, x: Option<i16>, y: Option<i16>, color: Option<i16>) {
	if let Some(x) = x {
...
pub fn cursor<A: Allocator + 'static>(rt: &mut Runtime<A>, x: Option<i16>, y: Option<i16>, col: Option<i16>) -> (u8, u8, u8) {
	let prev_cursor = *rt.memory.machine_state().cursor_position();
	let prev_color = *rt.memory.machine_state().pen_color();
	
	set_cursor_color(&mut rt.memory.machine_state(), x.or(Some(0)), y.or(Some(0)), col);
...
pub fn color<A: Allocator + 'static>(rt: &mut Runtime<A>, val: Option<u8>) -> u8 {
	let old = *rt.memory.machine_state().pen_color();
	if let Some(val) = val {
		*rt.memory.machine_state().pen_color() = val;
	}
	old
}
...
pub fn camera<A: Allocator + 'static>(rt: &mut Runtime<A>, x: Option<i16>, y: Option<i16>) -> (i16, i16) {
	let old = rt.memory.machine_state().get_camera_position();
...
pub fn clip<A: Allocator + 'static>(rt: &mut Runtime<A>, x: Option<u8>, y: Option<u8>, w: Option<u8>, h: Option<u8>, clip_previous: Option<bool>) -> (u8, u8, u8, u8) {
	let [x_begin_old, y_begin_old, x_end_old, y_end_old] = *rt.memory.machine_state().clip_rect();
	...
	if let Some(x) = x && let Some(y) = y && let Some(w) = w && let Some(h) = h {
		...
		*rt.memory.machine_state().clip_rect() = [x_begin, y_begin, x_end.min(128), y_end.min(128)];
	} else {
		*rt.memory.machine_state().clip_rect() = [0, 0, 128, 128];
	}
